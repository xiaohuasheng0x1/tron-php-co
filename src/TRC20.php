...
class TRC20 extends TRX
{
    ...
    public function balanceOf(Address $address)
    {
    ...
    public function transfer(Address $from, Address $to, float $amount, Address $root = null): Transaction
    {
        if (is_null($root)) {
            $root = $from;
        }
        $this->tron->setAddress($root->address);
        $this->tron->setPrivateKey($root->privateKey);
    ...
    public function transferFrom(Address $from, Address $to, Address $root, float $amount)
    {
